...
class DomoticaController extends Controller
{
    public function index()
    {
        ...
        $accidentLoop = DB::table('incidente')->get();
          ...
          // Enable to send alert email when camara detect intruder  
...
//            $user->notify(new EmailNotification($userAuth,User::first()));

//            $user->notify((new IntrunderAlert())->delay($when));
...
//        }
    ...
    public function changePassword(UpdateProfile $request )
    {
        ...
        public function sensorCode(PincodeCheck $request )
        {
            ...
            return response()->json(['success'=>'Añadido correctamente.']);
